function run_inventory_management_sim(
  ...
  function inventory_management_sim(n_buckets, inputs, model) {
    ...
    var capacities = [];
    if (model) {
      d3.select("#im2-capacities")
        .selectAll("rect")[0]
        .forEach(function (d) {
          capacities.push(+d.height.animVal.value);
        });
      demand_impact = 0.2 * 2.5;
      allocation_impact = 0.2 * 2.5;
      clearance_impact = 0.5 * 2.5;
    } else {
      d3.select("#im-capacities")
        .selectAll("rect")[0]
        .forEach(function (d) {
          capacities.push(90);
        });
    ...
    var line_g2 = svg2
      ...
      .attr(
        "transform",
        "translate(" + lines_margin.left + "," + lines_margin.top2 + ")"
      );

    var x = d3.scale.linear().domain([0, 200]).range([0, lines_width]);

    var y = d3.scale.linear().domain([0, 14]).range([lines_height, 0]);

    var inventory_area = d3.svg
      .area()
      .x(function (d) {
        return x(d.t);
      })
      .y0(y(0))
      .y1(function (d) {
        return y(d.inventory / 10);
      });

    var demand_line = d3.svg
      .line()
      .x(function (d) {
        return x(d.t);
      })
    ...
    function timer_function(elapsed) {
      ...
      if (n_buckets > 1) {
        for (var i = 0; i < 5; i++) {
          ...
          demand[i] = Math.min(
            5,
            Math.max(0, Math.min(capacities[i] / demand_impact, demand[i]))
          );

          var wait_for_clothes = false;
          if (
            inventory_management_clothing_animation_running &&
            elapsed < 2800
          ) {
          ...
          if (inputs && !wait_for_clothes) {
            ...
            if (
              ...
              clearance[i] = Math.max(0, clearance[i] - 1);
            }
            ...
            if (capacities[i] > 90) {
              allocation[i] = 0;
            } else if (capacities[i] < 20) {
              allocation[i] = 8;
            } else {
              allocation[i] +=
                (-1 + (capacities[i] < 70) * 2) * Math.random() * 0.2;
      ...
      if (model) {
        d3.select("#im2-capacities")
          ...
          .attr("y", function (d) {
            return 432.8 - d * 0.7 + 70;
          });

        d3.select("#im2-purchase")
          .select("line")
          .attr(
            "stroke-width",
            Math.max(1 * nonzero_replenish, Math.min(6, purchase / 1.5))
          );

        d3.select("#im2-allocation")
          .selectAll("line")
          .data(allocation)
        ...
        d3.selectAll(".im2-demand-return")
          .data(demand)
          .attr("stroke-width", function (d) {
            return 0.75 + d * 0.75;
          });

        d3.selectAll(".im2-demand-supply")
          .data(demand)
          .attr("stroke-width", function (d) {
            return 0.75 + d * 1.5 * 0.75;
          });
      } else {
        if (n_buckets == 1) {
          d3.select("#im-capacities-single")
            .selectAll("rect")
            .data([capacities[0]])
            .attr("height", function (d) {
              return d;
          ...
          d3.select("#im-purchase-single")
            .select("polyline")
            .attr("stroke-width", function (d) {
              if (
                elapsed < single_sku_big_dump_time + 400 &&
                single_sku_big_dump_time != -1
              ) {
                return 8;
              } else {
                return 0;
          ...
          d3.selectAll(".im-demand-supply-single")
            .data([demand[0]])
            .attr("stroke-width", function (d) {
              return 1 + d * 0.5;
            });
          ...
          d3.select("#im-purchase")
            .select("line")
            .attr(
              "stroke-width",
              Math.max(1 * nonzero_replenish, Math.min(8, purchase / 2))
            );

          d3.select("#im-allocation")
            .selectAll("line")
            .data(allocation)
            .attr("stroke-width", function (d) {
              return d;
          ...
          d3.selectAll(".im-demand-return")
            .data(demand)
            .attr("stroke-width", function (d) {
              return 1 + d;
            });

          d3.selectAll(".im-demand-supply")
            .data(demand)
            .attr("stroke-width", function (d) {
              return 1 + d * 1.5;
            });
    ...
    if (model) {
      animation_state.inventory_management_sim_model_timer =
  ...
  function inventory_management_sim_detailed() {
    var capacities = [];
    d3.select("#im-capacities-detailed")
      .selectAll("rect")[0]
      .forEach(function (d) {
        capacities.push(+d.height.animVal.value);
      });
    ...
    animation_state.inventory_management_sim_detailed_timer = d3.timer(
      function (elapsed) {
        if (elapsed > 100000) {
          stop_animation_timer("inventory_management_sim_detailed_timer");
        }
  ...
  var start_inventory_management_clothing_animation = function (single_sku) {
    ...
    // animation timer
    animation_state.inventory_management_clothing_animation_timer = d3.timer(
      function (t_ms) {
        if (t_ms > 1e5) {
          stop_animation_timer("inventory_management_clothing_animation_timer");
        }
        ...
        if (single_sku && single_sku_big_dump) {
          ...
          for (var i = 0; i < 2; i++) {
            if (Math.random() < 0.3) {
              ...
              if (sil_n == 5) {
                sil_n = 6 + Math.floor(Math.random() * 8);
              }
              particle_count += 1;
              current_events.push({
                particle_id: particle_count,
        ...
        inventory_items
          ...
          .attr("opacity", 1e-6)
          .attr("d", function (d) {
            return silhouette_outline_d(d.sil, 290, 0);
          });
        ...
        inventory_items
          .attr("d", function (d) {
            ...
            if (d.e1 == 0) {
              ...
              var new_rad =
                d.rad * (1 - rel_t) + (rel_t * (inner_r + outer_r)) / 2;
  ...
  function q_t_model_graph_axes() {
    ...
    var y2 = d3.scale.linear().domain([0, 240]).range([lines_height, 0]);
    ...
    var ya = line_g2.append("g").attr("class", "y axis");
    ya.append("line")
      .attr("x1", 0)
      .attr("x2", 0)
      .attr("y1", y2(0))
    ...
    var ya2 = line_g2.append("g").attr("class", "y axis");
    ya2
      .append("line")
      .attr("x1", x(200))
      .attr("x2", x(200))

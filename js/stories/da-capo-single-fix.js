function run_single_fix_overview_animation() {
  ...
  for (var i = 0; i < 5; i++) {
    wh.push({
      x: wh_x + Math.random() * wh_width,
      y: wh_y + Math.random() * wh_height,
    });
  }
  ...
  function single_fix_sim(sim_number, speed) {
    animation_state.single_fix_sim_timer = d3.timer(function (t) {
      ...
      Object.keys(single_fix_elements).forEach(function (el_name) {
        ...
        if (i == -2) {
          // remove element(s)
          d3.select("#single-fix-sim")
            .selectAll(".single-fix-sim-" + el_name + "-" + sim_number)
          ...
          if (i == 0) {
            ...
            if (el.type == "path") {
              ...
              d3.select("#single-fix-sim")
                ...
                .attr(
                  "d",
                  silhouette_outline_d(el.form_n, tx + el.dx, ty + el.dy)
                );
            }
          }
          // move element(s)
          ...
          if (el.type == "circle") {
            if (el.progression[i].i == "linear") {
              ...
              d3.select("#single-fix-sim")
                ...
                .attr(
                  "cx",
                  el.progression[i].cx * (1 - de) +
                    el.progression[i + 1].cx * de
                )
                .attr(
                  "cy",
                  el.progression[i].cy * (1 - de) +
                    el.progression[i + 1].cy * de
                );
            }
          ...
          if (el.type == "path") {
            if (el.progression[i].i == "linear") {
              ...
              var tx =
                ...
                (el.dx + el.progression[i + 1].x) * de;
              var ty =
                el.progression[i].y * (1 - de) +
                (el.dy + el.progression[i + 1].y) * de;
              d3.select("#single-fix-sim")
                .selectAll(".single-fix-sim-" + el_name + "-" + sim_number)
  ...
  function single_fix_sim_animate_labels() {
    d3.selectAll(
      ".single-fix-sim-persistent-lines, .single-fix-sim-persistent-text"
    )
      .transition()
      .duration(0)
      .remove();
    ...
    // request
    d3.select("#single-fix-sim-labels")
      .append("path")
    ...
    d3.selectAll(".single-fix-sim-persistent-text").style("opacity", 0);

    d3.selectAll(
      ".request-line, .to-logistics-line, .delivery-line, .return-line, .feedback-line"
    )
      .transition()
      .duration(0)
      .attr("stroke-dashoffset", function (d) {
    ...
    d3.selectAll(
      ".request-text, .to-logistics-text, .delivery-text, .return-text, .feedback-text"
    )
      .transition()
      .duration(0)
      .style("opacity", 0);
  ...
  d3.select("#icycle-warehouses").select("rect").attr("fill", "none");
}

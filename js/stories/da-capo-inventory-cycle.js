...
function start_inventory_animation() {
  if (animation_state.inventory_animation_timer == false) {
    ...
    d3.select("#icycle-warehouses").select("rect").attr("fill", "#fff");
    ...
    var coord_x_out = function (r, a) {
      return center_x + r * Math.cos(a);
    };
    var coord_y_out = function (r, a) {
      return center_y_out + r * Math.sin(a);
    };
    ...
    animation_state.inventory_animation_timer = d3.timer(function (t_ms) {
      ...
      inventory_items
        ...
        .attr("d", function (d) {
          ...
          if (d.e1 == 0) {
            ...
            if (
              coord_y_in(new_rad, angle_in(t, d.t1, d.t2, d.e1, d.e2)) >
              center_y_main - outer_r + new_rad
            ) {
      ...
      for (
        var learning_i = 0;
        learning_i < learning_events_config.length;
        learning_i++
      ) {
        if (
          Math.random() < learning_events_config[learning_i].p &&
          t_ms > learning_events_config[learning_i].enter_time
        ) {
      ...
      learning_items = svg
        .selectAll(".learning-item")
        .data(learning_current_events, function (d) {
          return d.id;
        });
...
function draw_inventory_cycle_animation_labels() {
  // feedback
  d3.select("#inventory-cycle-sim-labels")
    .append("text")

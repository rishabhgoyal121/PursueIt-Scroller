...
for (var i = 0; i < 10; i++) {
  ...
  overview_states.push({
    x: overview_state_locations[k].x - 10 + 10 * Math.random() - 10 * i,
    y: overview_state_locations[k].y - 10 + 10 * Math.random(),
  });
}

var overview_styles_overlap = [
  { x: 0, y: 12 },
  { x: -10, y: -43 },
  { x: -55, y: -43 },
  { x: -30, y: -5 },
...
for (var i = 0; i < 8; i++) {
  var k = Math.floor(Math.random() * 3);
  overview_style_locations.push({
    x:
      overview_wh_locations[k].x +
      100 * Math.random() +
      overview_styles_overlap[i].x,
...
function draw_overview_sneakpeek_labels() {
  d3.select("#sketch-animation-labels")
    .selectAll("circle")
...
function overview_animation_run() {
  function overview_incoming_clothes() {
    ...
    d3.select("#sketch-animation")
      ...
      .attr("d", function (d) {
        return silhouette_outline_d(d.form_n, 500, 180);
      })
      .transition()
      .delay(function (d) {
        return d.delay1;
      ...
      .duration(2000)
      .attr("d", function (d) {
        return silhouette_outline_d(d.form_n, d.x1, d.y1);
      })
      .transition()
      .delay(function (d) {
        return d.delay2;
  ...
  function overview_state_machine() {
    ...
    d3.selectAll("#sketch-clients, #sketch-client-selected")
      .selectAll("g")
  ...
  function overview_recsys_animation() {
    ...
    for (var k = 0; k < 4; k++) {
      ...
      for (var i = 0; i < 4; i++) {
        p.push(
          d3
            .select("#sketch-stylingAlgo")
            .select(".sketch_recsys_arc_" + k + "_" + i)
            .datum()
        );
    ...
    var this_timer = d3.timer(function (local_t) {
      if (local_t > transition_duration) {
        ...
        for (var k = 0; k < 4; k++) {
          for (var i = 0; i < 4; i++) {
            d3.select("#sketch-stylingAlgo")
              .select(".sketch_recsys_arc_" + k + "_" + i)
              .datum({
                startAngle:
                  previous_datum[k][i].startAngle +
                  ((new_datum[k][i].startAngle -
                    previous_datum[k][i].startAngle) *
                    local_t) /
                    transition_duration,
                endAngle:
                  previous_datum[k][i].endAngle +
                  ((new_datum[k][i].endAngle - previous_datum[k][i].endAngle) *
                    local_t) /
                    transition_duration,
  ...
  function overview_recsys_draw() {
    ...
    for (var k = 0; k < 4; k++) {
      for (var i = 0; i < 4; i++) {
        ...
        d3.select("#sketch-stylingAlgo")
          .selectAll(".sketch_recsys_arc_" + k + "_" + i)
          .remove();
  ...
  if (animation_state.overview_animation_interval == false) {
    ...
    animation_state.overview_animation_interval = setInterval(
      overview_state_machine,
      1100
    );
    overview_recsys_draw();
    //overview_recsys_animation()
    animation_state.overview_animation_interval2 = setInterval(
      overview_recsys_animation,
      5000
    );
  }
...
function overview_reset() {
  ...
  d3.selectAll(
    "#sketch-inventory, #sketch-clients, #sketch-client-selected, #sketch-styling-circle"
  )
    .transition()
    .duration(0)
    .attr("display", "block")
...
function overview_sneakpeek(progress) {
  ...
  function overview_unveil(progress) {
    ...
    d3.select("#sketch-inventory")
      ...
      .duration(0)
      .attr("transform", function (d) {
        return "translate(" + d.x * progress + "," + d.y * progress + ")";
      });
  ...
  if (progress < 0.1) {
    ...
    overview_hide(
      "#sketch-stylingAlgo, #sketch-animation-labels, #sketch-animation"
    );
    overview_unveil(0);
    ...
    overview_hide(
      "#sketch-styling-circle, #sketch-stylingAlgo, #sketch-animation-labels, #sketch-animation"
    );
    overview_unveil((progress - 0.1) / 0.25);
    ...
    overview_show(
      "#sketch-stylist, #sketch-machines, #sketch-stylingAlgo, #sketch-animation-labels, #sketch-animation"
    );
    overview_unveil(1);

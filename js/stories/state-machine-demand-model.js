...
function state_transitions(show_history) {
  ...
  var color = d3.scale
    .ordinal()
    .range(["#aaa", "#CCDE66", "#F3A54A", "#4B90A6", "#AA7CAA"]);
  var color_labels = d3.scale
    .ordinal()
    .range(["#aaa", "#CCDE66", "#F3A54A", "#4B90A6", "#AA7CAA"]);
  ...
  var y = d3.scale.linear().domain([0, 600]).range([lines_height, 0]);
  ...
  var line_g = svg
    ...
    .attr(
      "transform",
      "translate(" + lines_margin.left + "," + lines_margin.top + ")"
    );
  ...
  if (show_history) {
    ...
    var ya = line_g.append("g").attr("class", "y axis");
    ya.append("line")
      .attr("x1", 0)
      .attr("x2", 0)
      .attr("y1", y(0))
  ...
  var source_i = foci.findIndex(function (d) {
      return d.type == "source";
    }),
    sink_i = foci.findIndex(function (d) {
      return d.type == "sink";
    });

  var nodes = [];

  animation_state.state_machine_animation_force = d3.layout
    .force()
    .nodes(nodes)
    .links([])
    .gravity(0)
  ...
  var next_month = function () {
    ...
    if (t > 199) {
      ...
      nodes.forEach(function (o, i) {
        ...
        var cum_trans_probs = [];
        trans_probs.reduce(function (a, b, i) {
          return (cum_trans_probs[i] = a + b);
        }, 0);
        var r = Math.random();
        var new_state = cum_trans_probs.findIndex(function (d) {
          return d >= r;
        });
        if (new_state != -1) {
      ...
      if (t > 1) {
        histry.push({
          ...
          demand:
            focus_node_count[1] * p_fix[0] +
            focus_node_count[2] * p_fix[1] +
            focus_node_count[3] * p_fix[2],
        });
      }
      ...
      var num_new = 100;
      if (t > 1) {
        num_new = Math.floor(Math.random() * 5);
      }
      if (t > 200) {
        num_new = 0;
      }
      ...
      for (var i = 0; i < num_new; i++) {
        nodes.push({
          id: n,
          state: source_i,
          previous_state: source_i,
          x: foci[source_i].x + 10 * Math.random(),
          y: foci[source_i].y + 10 * Math.random(),
        });
        n += 1;
      }
      ...
      if (show_history) {
        ...
        var state_history = line_g.selectAll(".state_history").data(histry);

        state_history.enter().append("g").attr("class", "state_history");
        ...
        state_history
          ...
          .attr("y", function (d) {
            return y(d.n1 + d.n2 + d.n3);
          })
          .attr("width", 2.5)
          .attr("height", function (d) {
            return y(d.n1 + d.n2) - y(d.n1 + d.n2 + d.n3);
          })
          .style("fill", color(3));
  ...
  animation_state.state_machine_animation_interval = setInterval(
    next_month,
    500
  );

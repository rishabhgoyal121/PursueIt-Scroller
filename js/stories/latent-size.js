function animated_fit_learning_narrative(current_narrative_position) {
  ...
  var client_bodies = svg
    .selectAll(".client")
    .data(clients_visible, function (d) {
      return d.id;
    })
    .enter()
    .append("g")
    ...
    .attr(
      "transform",
      "translate(" + (350 - 283.5) + "," + (130 - 399.1 - 12) + ")"
    );
  ...
  client_bodies
    ...
    .attr(
      "d",
      "M288.7,409.4v8.2c-0.7,0.3-1.5,0.6-2.2,0.8c-0.9,0.2-1.9,0.4-2.9,0.4c-1,0-2-0.1-2.9-0.4c-0.8-0.2-1.5-0.4-2.2-0.8v-8.2c0-2.8,2.3-5.2,5.2-5.2C286.4,404.2,288.7,406.5,288.7,409.4z"
    )
    .style("fill", "#52598B");

  svg
    .selectAll(".style")
    .data(styles_visible, function (d) {
      return d.id;
    })
    .enter()
    .append("g")
  ...
  function animation_step() {
    ...
    for (var i = 0; i < n_pairs; i++) {
      ...
      var weights = styles.map(function (d) {
        return d.current_position - clients[client_id].current_position;
      });
      var cum_weights = [];
      weights.reduce(function (a, b, i) {
        cum_weights[i] = { v: a + b, id: i };
        return a + b;
      }, 0);
      cum_weights = cum_weights.sort(function (a, b) {
        return a.v > b.v;
      });
      ...
      if (cum_weights[cum_weights.length - 1].v != 0) {
        ...
        var sel = cum_weights.find(function (d) {
          return d.v >= sel_random;
        });
        if (!(sel == null)) {
          style_id = sel.id;
        }
      ...
      var feedback =
        -1 +
        2 * (clients[client_id].latent_size > styles[style_id].latent_size);

      // use feedback if it contradicts current
      if (
        feedback == -1 &&
        clients[client_id].current_position <= styles[style_id].current_position
      ) {
        clients[client_id].next_position =
          clients[client_id].current_position + Math.random() * learning_rate;
        styles[style_id].next_position =
          styles[style_id].current_position - Math.random() * learning_rate;
      }
      ...
      clients[client_id].next_position = Math.min(
        1,
        Math.max(0, clients[client_id].next_position)
      );
      styles[style_id].next_position = Math.min(
        1,
        Math.max(0, styles[style_id].next_position)
      );
    }
    ...
    for (var i = 0; i < hist_n; i++) {
      ...
      clients.forEach(function (d) {
        if (
          d.current_position >= i / hist_n &&
          d.current_position < (i + 1) / hist_n
        ) {
          this_count += 1;
        }
      });
    ...
    pairs = pairs.filter(function (d) {
      var keep = true;
      if (d.client_id >= n_clients_visible) {
        keep = false;
      }
      if (d.style_id >= n_styles_visible) {
        keep = false;
      }
      return keep;
    });
    ...
    svg
      ...
      .style("fill", "none")
      .attr("x1", function (d) {
        return x(clients[d.client_id].current_position);
      })
      .attr("x2", function (d) {
        return x(styles[d.style_id].current_position);
      })
      .transition()
      .delay(delay)
      .duration(move)
      .attr("x1", function (d) {
        return x(clients[d.client_id].next_position);
      })
      .attr("x2", function (d) {
        return x(styles[d.style_id].next_position);
      });
    ...
    svg
      ...
      .duration(move)
      .attr("transform", function (d) {
        return "translate(" + (x(d.next_position) - 350) + ",0)";
      });
    ...
    if (show_uncertainty) {
      svg
        ...
        .attr("height", 6)
        .attr("x", function (d) {
          return x(d.current_position) - 150 / Math.sqrt(1 + d.current_n_tests);
        })
        .attr("width", function (d) {
          return 300 / Math.sqrt(1 + d.current_n_tests);
        })
        .transition()
        .delay(delay)
        .duration(move)
        .attr("x", function (d) {
          return x(d.next_position) - 150 / Math.sqrt(1 + d.next_n_tests);
        })
        .attr("width", function (d) {
          return 300 / Math.sqrt(1 + d.next_n_tests);
        });

      svg
        .selectAll(".style_uncertainty")
        .data(styles_visible, function (d) {
          return d.id;
        })
        .enter()
    ...
    if (show_histogram) {
      svg
        ...
        .attr("y", function (d) {
          return 120 - Math.min(60, client_hist_dy * d);
        })
        .attr("width", 300 / hist_n)
        .attr("height", function (d) {
          return Math.min(60, client_hist_dy * d);
        });

      svg
        .selectAll(".style_hist")
        .data(style_hist)
        .enter()
        .append("rect")
        ...
        .attr("width", 300 / hist_n)
        .attr("height", function (d) {
          return Math.min(60, style_hist_dy * d);
        });
    }
    ...
    animation_state.latent_size_animation_interval = setTimeout(
      animation_step,
      delay + move + wait
    );
  }
...
function latent_size_animation_1() {
  ...
  animation_state.latent_size_meta1_interval = setTimeout(function () {
    animated_fit_learning_narrative(2);
  }, 3000);
}

function latent_size_animation_2() {
  animated_fit_learning_narrative(3);
  animation_state.latent_size_meta2_interval = setTimeout(function () {
    animated_fit_learning_narrative(4);
  }, 5000);
}

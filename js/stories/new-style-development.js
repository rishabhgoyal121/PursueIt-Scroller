  ...
  d3.select("#nsd-attrs-blouse")
    ...
    .attr("transform", function (d) {
      if (progress2 < 7000) {
        ...
        return (
          "translate(" + x_end + "," + y_end + ") scale(" + scale_end + ")"
        );
      }
    });
...
function create_frankenstyle() {
  ...
  d3.select("#nsd-frankenstyle-dev-color")
    .select("circle")
    .attr("fill", color_options[color_choice]);

  d3.select("#nsd-frankenstyle-dev-greyscale")
    .select("circle")
    .attr(
      "fill",
      greyscale_options[Math.floor(Math.random() * greyscale_options.length)]
    );

  d3.select("#nsd-frankenstyle-dev-pattern")
    .select("image")
    .attr("xlink:href", "img/styledev/" + pattern_options[pattern_choice]);

  d3.select("#nsd-frankenstyle-dev-neckline")
    .select("path")
    .attr("d", neckline_options[neckline_choice]);

  d3.select("#nsd-frankenstyle-dev-sleeve")
    .select("path")
    .attr("d", sleeve_options[sleeve_choice]);

  d3.select("#nsd-frankenstyle-dev-squiggle")
    .select("path")
    .attr(
      "d",
      squiggle_options[Math.floor(Math.random() * squiggle_options.length)]
    );

  d3.select("#nsd-frankenstyle-dev-sizeratio")
    .select("g")
    .select("text")
    .text((Math.random() * 2).toFixed(2));

  d3.select("#nsd-frankenstyle-dev-number")
    .select("text")

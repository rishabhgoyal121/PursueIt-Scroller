function run_platform_animation() {
  ...
  var data_dots_start = data_dots_given[0].map(function (d) {
    ...
    var y = transform_text
      .substring(0, transform_text.length - 1)
      .split(" ")[5];
  ...
  platform_animation_lambdas.push({
    layer: "platform-animation",
    id: "g0a",
    script: "λ",
    subscript: "1",
    rad: 5,
  ...
  function datanado(
    ...
    var data_circles = d3
      .select("#platform-animation-datanado")
      .selectAll("text")
    ...
    data_circles.attr("transform", function (d) {
      var x, y;
      if (delta_t < 0.8) {
      ...
      if (delta_t < 0.9) {
        ...
        y = Math.min(
          y_max,
          Math.max(y_min, (1 - delta_t) * d.y + delta_t * y_center)
        );
      } else {
  ...
  function start_platform_animation() {
    ...
    animation_state.data_platform_animation_timer = d3.timer(function (
      timer_t
    ) {
      ...
      var text_progress_1 = Math.max(
        0,
        Math.min(
          load_my_data_text.length + 1,
          Math.floor((given_t / 800) * load_my_data_text.length)
        )
      );
      var text_progress_2 = Math.max(
        0,
        Math.min(
          run_my_algorithm_text.length + 1,
          Math.floor(((given_t - 1000) / 800) * run_my_algorithm_text.length)
        )
      );
      var text_progress_3 = Math.max(
        0,
        Math.min(
          deploy_to_production_text.length + 1,
          Math.floor(
            ((given_t - 2000) / 800) * deploy_to_production_text.length
          )
        )
      );

      d3.select("#load_my_data").text(
        load_my_data_text.substring(0, text_progress_1)
      );

      d3.select("#run_my_algorithm").text(
        run_my_algorithm_text.substring(0, text_progress_2)
      );

      d3.select("#deploy_to_production").text(
        deploy_to_production_text.substring(0, text_progress_3)
      );

      // ingestion

      if (Math.random() < 0.5) {
        d3.select("#platform-animation-ingestion")
          .append("text")
          .attr("x", 1100)
          .attr("y", 503)
          .style(
            "fill",
            data_dots_colors[
              Math.floor(Math.random() * data_dots_colors.length)
            ]
          )
      ...
      if (Math.random() < 0.2) {
        if (given_t > last_api_call + 5000) {
          ...
          d3.select("#platform-animation-api")
            .append("text")
            .attr("x", 1100)
            .attr("y", 418)
            .style("fill", "#000")
      ...
      platform_animation_lambdas.forEach(function (l) {
        if (t > l.progression[0].t) {
          ...
          var entered = lambda
            ...
            .attr(
              "transform",
              "translate(" + l.progression[0].x + "," + l.progression[0].y + ")"
            );
          ...
          var i =
            -1 +
            l.progression.findIndex(function (d) {
              return t <= d.t;
            });
          ...
          if (i == -2) {
            ...
            var ldt =
              (t - l.progression[i].t) /
              (l.progression[i + 1].t - l.progression[i].t);
            var x =
              l.progression[i].x * (1 - ldt) + l.progression[i + 1].x * ldt;
            var y =
              l.progression[i].y * (1 - ldt) + l.progression[i + 1].y * ldt;

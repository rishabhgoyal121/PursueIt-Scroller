function remove_simple_pairs_stuff(prefix) {
  d3.select("#" + prefix + "-greenbox")
    .transition()
    .duration(200)
    .style("opacity", 0);
  d3.select("#" + prefix + "-blueboxes")
    .select("rect")
    .remove();
  d3.select("#" + prefix + "-greenarrows")
...
function remove_complex_pairs_stuff(prefix) {
  d3.select("#" + prefix + "-blueboxes")
    .select("rect")
    .remove();
  d3.select("#" + prefix + "-orangebox")
    .select("rect")
    .remove();
  d3.select("#" + prefix + "-greenarrows")
    .selectAll("path, polygon")
    .remove();
  d3.select("#" + prefix + "-orangearrows")
    .selectAll("path, polygon")
    .remove();
}

function scroll_through_simple_pairs(
  prefix,
  state_running_prop,
  interval_length,
  transition_length,
...
function scroll_through_complex_pairs(
  prefix,
  state_running_prop,
  interval_length,
  transition_length,
